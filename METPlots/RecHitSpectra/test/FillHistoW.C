...
41 4.889 5.191*/
  ...
  if(!strcmp(wfile,"none")){
	...
	if(inWeightFile->IsZombie()){
		cout << "Root file: " << wfile << " not found!" << endl;
		return;
	}
	TH1D *h_weights = (TH1D*)inWeightFile->Get("h_nVerticesWeights");
	if(!h_weights){
		cout << "Could not load weight histogram, h_nVerticesWeights!" << endl;
		return;
	}
	for(int i = 0; i <= nMaxVertx; i++){
		weights[i] = h_weights->GetBinContent(i+1);
	}
	cout << "Finished loading weights from file." << endl;
	inWeightFile->Close();
  }
  ...
  double nRecHitsHB1 = 0;
  double nRecHitsHB2 = 0;

  double nRecHitsHE1 = 0;
  double nRecHitsHE2 = 0;
  double nRecHitsHE3 = 0;

  double nRecHitsHF1 = 0;
  double nRecHitsHF2 = 0;
  ...
  */
  ...
  double w; //This is the weight to assign to the event
  
  while(tReader->Next()){
    w = weights[*numVertices];	
    numRecHits = 0;
    ...
    */
    ...
    nRecHitsHB2 = 0;
     for(int ridx = 0; ridx < (*readRecHitsHB2_En).size(); ridx++){
      ...
      recHitEnergyHB2->Fill((*readRecHitsHB2_En).at(ridx),w);
      recHitEnergyHB2s->Fill((*readRecHitsHB2_En).at(ridx),w);
      recHitEnergyHB->Fill((*readRecHitsHB2_En).at(ridx),w);
      recHitEnergyHBs->Fill((*readRecHitsHB2_En).at(ridx),w);

      recHitEnergyRAWHB2->Fill((*readRecHitsHB2_EnRAW).at(ridx),w);
      recHitEnergyRAWHB2s->Fill((*readRecHitsHB2_EnRAW).at(ridx),w);
      recHitEnergyRAWHB->Fill((*readRecHitsHB2_EnRAW).at(ridx),w);
      recHitEnergyRAWHBs->Fill((*readRecHitsHB2_EnRAW).at(ridx),w);
 
      if((*readRecHitsHB2_EnRAW).at(ridx) > 0.1) recHitHBM0M2->Fill((*readRecHitsHB2_EnRAW).at(ridx),(*readRecHitsHB2_En).at(ridx),w);
      if((*readRecHitsHB2_EnRAW).at(ridx) > 0.1) recHitHBM0M2Ratio->Fill((*readRecHitsHB2_EnRAW).at(ridx),(*readRecHitsHB2_En).at(ridx)/(*readRecHitsHB2_EnRAW).at(ridx),w);
      if((*readRecHitsHB2_EnRAW).at(ridx) > 0.1) recHitHBM2M0Dist->Fill((*readRecHitsHB2_EnRAW).at(ridx),(*readRecHitsHB2_En).at(ridx)/(*readRecHitsHB2_EnRAW).at(ridx),w);

      recHitTimingHB2->Fill((*readRecHitsHB2_time).at(ridx),w);
      recHitTimingHB2s->Fill((*readRecHitsHB2_time).at(ridx),w);

      recHitOccupancyHB2->Fill((*readRecHitsHB2_ieta).at(ridx),(*readRecHitsHB2_iphi).at(ridx),w);
      recHitTimingMapHB2->Fill((*readRecHitsHB2_ieta).at(ridx),(*readRecHitsHB2_iphi).at(ridx),(*readRecHitsHB2_time).at(ridx),w);
      recHitEnergyMapHB2->Fill((*readRecHitsHB2_ieta).at(ridx),(*readRecHitsHB2_iphi).at(ridx),(*readRecHitsHB2_En).at(ridx),w);

      if((*readRecHitsHB2_En).at(ridx) > 0.7){
	recHitEnergyCutHB->Fill((*readRecHitsHB2_En).at(ridx),w);
	recHitEnergyCutHBs->Fill((*readRecHitsHB2_En).at(ridx),w);
      }
      ...
      if(enRaw > 5){
	if(recHitIeta > 0) recHitHBMRatio5plus->Fill(recHitIeta,MRatio,w);
	if(recHitIeta < 0) recHitHBMRatio5minus->Fill(-recHitIeta,MRatio,w);
      }

      if(enRaw > 10){
	if(recHitIeta > 0) recHitHBMRatio10plus->Fill(recHitIeta,MRatio,w);
	if(recHitIeta < 0) recHitHBMRatio10minus->Fill(-recHitIeta,MRatio,w);
      }

      if(enRaw > 20){
	if(recHitIeta > 0) recHitHBMRatio20plus->Fill(recHitIeta,MRatio,w);
	if(recHitIeta < 0) recHitHBMRatio20minus->Fill(-recHitIeta,MRatio,w);
      }

      recHitNum_ieta->Fill(recHitIeta,w);
      recHitNum_ieta_s->Fill(recHitIeta,w);
      if(recHitEnergy > 0.7){
	recHitCutNum_ieta->Fill(recHitIeta,w);
	recHitCutNum_ieta_s->Fill(recHitIeta,w);
      }
      if(recHitIeta > 0){
	...
	if(recHitEnergy > 0.7){
     ...
     h_nRecHitsHB2->Fill(nRecHitsHB2,w);
     h_nRecHitsHB2s->Fill(nRecHitsHB2,w);
    ...
    nRecHitsHB1 = 0;
     for(int ridx = 0; ridx < (*readRecHitsHB1_En).size(); ridx++){
      ...
      recHitEnergyHB1->Fill((*readRecHitsHB1_En).at(ridx),w);
      recHitEnergyHB1s->Fill((*readRecHitsHB1_En).at(ridx),w);
      recHitEnergyHB->Fill((*readRecHitsHB1_En).at(ridx),w);
      recHitEnergyHBs->Fill((*readRecHitsHB1_En).at(ridx),w);

      recHitEnergyRAWHB1->Fill((*readRecHitsHB1_EnRAW).at(ridx),w);
      recHitEnergyRAWHB1s->Fill((*readRecHitsHB1_EnRAW).at(ridx),w);
      recHitEnergyRAWHB->Fill((*readRecHitsHB1_EnRAW).at(ridx),w);
      recHitEnergyRAWHBs->Fill((*readRecHitsHB1_EnRAW).at(ridx),w);

      if((*readRecHitsHB1_EnRAW).at(ridx) > 0.1) recHitHBM0M2->Fill((*readRecHitsHB1_EnRAW).at(ridx),(*readRecHitsHB1_En).at(ridx),w);
      if((*readRecHitsHB1_EnRAW).at(ridx) > 0.1) recHitHBM0M2Ratio->Fill((*readRecHitsHB1_EnRAW).at(ridx),(*readRecHitsHB1_En).at(ridx)/(*readRecHitsHB1_EnRAW).at(ridx),w);
      if((*readRecHitsHB1_EnRAW).at(ridx) > 0.1) recHitHBM2M0Dist->Fill((*readRecHitsHB1_EnRAW).at(ridx),(*readRecHitsHB1_En).at(ridx)/(*readRecHitsHB1_EnRAW).at(ridx),w);

      recHitTimingHB1->Fill((*readRecHitsHB1_time).at(ridx),w);
      recHitTimingHB1s->Fill((*readRecHitsHB1_time).at(ridx),w);

      recHitOccupancyHB1->Fill((*readRecHitsHB1_ieta).at(ridx),(*readRecHitsHB1_iphi).at(ridx),w);
      recHitTimingMapHB1->Fill((*readRecHitsHB1_ieta).at(ridx),(*readRecHitsHB1_iphi).at(ridx),(*readRecHitsHB1_time).at(ridx),w);
      recHitEnergyMapHB1->Fill((*readRecHitsHB1_ieta).at(ridx),(*readRecHitsHB1_iphi).at(ridx),(*readRecHitsHB1_En).at(ridx),w);

      if((*readRecHitsHB1_En).at(ridx) > 0.7){
	recHitEnergyCutHB->Fill((*readRecHitsHB1_En).at(ridx),w);
	recHitEnergyCutHBs->Fill((*readRecHitsHB1_En).at(ridx),w);
      }
	...
	if(recHitEnergy > 0.7){
     ...
     h_nRecHitsHB1->Fill(nRecHitsHB1,w);
     h_nRecHitsHB1s->Fill(nRecHitsHB1,w);

     h_nRecHitsHB->Fill(nRecHitsHB,w);
     h_nRecHitsHBs->Fill(nRecHitsHB,w);
    ...
    */
    ...
    nRecHitsHE1 = 0;
     for(int ridx = 0; ridx < (*readRecHitsHE1_En).size(); ridx++){
      ...
      recHitEnergyHE1->Fill((*readRecHitsHE1_En).at(ridx),w);
      recHitEnergyHE1s->Fill((*readRecHitsHE1_En).at(ridx),w);
      recHitEnergyHE->Fill((*readRecHitsHE1_En).at(ridx),w);
      recHitEnergyHEs->Fill((*readRecHitsHE1_En).at(ridx),w);
      recHitTimingHE1->Fill((*readRecHitsHE1_time).at(ridx),w);

      recHitEnergyRAWHE1->Fill((*readRecHitsHE1_EnRAW).at(ridx),w);
      recHitEnergyRAWHE1s->Fill((*readRecHitsHE1_EnRAW).at(ridx),w);
      recHitEnergyRAWHE->Fill((*readRecHitsHE1_EnRAW).at(ridx),w);
      recHitEnergyRAWHEs->Fill((*readRecHitsHE1_EnRAW).at(ridx),w);

      if((*readRecHitsHE1_EnRAW).at(ridx) > 0.1) recHitHEM0M2->Fill((*readRecHitsHE1_EnRAW).at(ridx),(*readRecHitsHE1_En).at(ridx),w);
      if((*readRecHitsHE1_EnRAW).at(ridx) > 0.1) recHitHEM0M2Ratio->Fill((*readRecHitsHE1_EnRAW).at(ridx),(*readRecHitsHE1_En).at(ridx)/(*readRecHitsHE1_EnRAW).at(ridx),w);
      if((*readRecHitsHE1_EnRAW).at(ridx) > 0.1) recHitHEM2M0Dist->Fill((*readRecHitsHE1_EnRAW).at(ridx),(*readRecHitsHE1_En).at(ridx)/(*readRecHitsHE1_EnRAW).at(ridx),w);

      recHitTimingHE1s->Fill((*readRecHitsHE1_time).at(ridx),w);

      recHitOccupancyHE1->Fill((*readRecHitsHE1_ieta).at(ridx),(*readRecHitsHE1_iphi).at(ridx),w);
      recHitTimingMapHE1->Fill((*readRecHitsHE1_ieta).at(ridx),(*readRecHitsHE1_iphi).at(ridx),(*readRecHitsHE1_time).at(ridx),w);
      recHitEnergyMapHE1->Fill((*readRecHitsHE1_ieta).at(ridx),(*readRecHitsHE1_iphi).at(ridx),(*readRecHitsHE1_En).at(ridx),w);
      if((*readRecHitsHE1_En).at(ridx) > 0.8){
	recHitEnergyCutHE->Fill((*readRecHitsHE1_En).at(ridx),w);
	recHitEnergyCutHEs->Fill((*readRecHitsHE1_En).at(ridx),w);
      }
      ...
      if(enRaw > 5){
        if(recHitIeta > 0) recHitHEMRatio5plus->Fill(recHitIeta,MRatio,w);
        if(recHitIeta < 0) recHitHEMRatio5minus->Fill(-recHitIeta,MRatio,w);
      }

      if(enRaw > 10){
        if(recHitIeta > 0) recHitHEMRatio10plus->Fill(recHitIeta,MRatio,w);
        if(recHitIeta < 0) recHitHEMRatio10minus->Fill(-recHitIeta,MRatio,w);
      }

      if(enRaw > 20){
        if(recHitIeta > 0) recHitHEMRatio20plus->Fill(recHitIeta,MRatio,w);
        if(recHitIeta < 0) recHitHEMRatio20minus->Fill(-recHitIeta,MRatio,w);
      }
    ...
    h_nRecHitsHE1->Fill(nRecHitsHE1,w);
    h_nRecHitsHE1s->Fill(nRecHitsHE1,w);
    ...
    nRecHitsHE2 = 0;
     for(int ridx = 0; ridx < (*readRecHitsHE2_En).size(); ridx++){
      ...
      recHitEnergyHE2->Fill((*readRecHitsHE2_En).at(ridx),w);
      recHitEnergyHE2s->Fill((*readRecHitsHE2_En).at(ridx),w);
      recHitEnergyHE->Fill((*readRecHitsHE2_En).at(ridx),w);
      recHitEnergyHEs->Fill((*readRecHitsHE2_En).at(ridx),w);

      recHitEnergyRAWHE2->Fill((*readRecHitsHE2_EnRAW).at(ridx),w);
      recHitEnergyRAWHE2s->Fill((*readRecHitsHE2_EnRAW).at(ridx),w);
      recHitEnergyRAWHE->Fill((*readRecHitsHE2_EnRAW).at(ridx),w);
      recHitEnergyRAWHEs->Fill((*readRecHitsHE2_EnRAW).at(ridx),w);

      if((*readRecHitsHE2_EnRAW).at(ridx) > 0.1) recHitHEM0M2->Fill((*readRecHitsHE2_EnRAW).at(ridx),(*readRecHitsHE2_En).at(ridx),w);
      if((*readRecHitsHE2_EnRAW).at(ridx) > 0.1) recHitHEM0M2Ratio->Fill((*readRecHitsHE2_EnRAW).at(ridx),(*readRecHitsHE2_En).at(ridx)/(*readRecHitsHE2_EnRAW).at(ridx),w);
      if((*readRecHitsHE2_EnRAW).at(ridx) > 0.1) recHitHEM2M0Dist->Fill((*readRecHitsHE2_EnRAW).at(ridx),(*readRecHitsHE2_En).at(ridx)/(*readRecHitsHE2_EnRAW).at(ridx),w);

      recHitTimingHE2->Fill((*readRecHitsHE2_time).at(ridx),w);
      recHitTimingHE2s->Fill((*readRecHitsHE2_time).at(ridx),w);

      recHitOccupancyHE2->Fill((*readRecHitsHE2_ieta).at(ridx),(*readRecHitsHE2_iphi).at(ridx),w);
      recHitTimingMapHE2->Fill((*readRecHitsHE2_ieta).at(ridx),(*readRecHitsHE2_iphi).at(ridx),(*readRecHitsHE2_time).at(ridx),w);
      recHitEnergyMapHE2->Fill((*readRecHitsHE2_ieta).at(ridx),(*readRecHitsHE2_iphi).at(ridx),(*readRecHitsHE2_En).at(ridx),w);
      if((*readRecHitsHE2_En).at(ridx) > 0.8){
	recHitEnergyCutHE->Fill((*readRecHitsHE2_En).at(ridx),w);
	recHitEnergyCutHEs->Fill((*readRecHitsHE2_En).at(ridx),w);
      }
    ...
    h_nRecHitsHE2->Fill(nRecHitsHE2,w);
    h_nRecHitsHE2s->Fill(nRecHitsHE2,w);
    ...
    nRecHitsHE3 = 0;
     for(int ridx = 0; ridx < (*readRecHitsHE3_En).size(); ridx++){
      ...
      recHitEnergyHE3->Fill((*readRecHitsHE3_En).at(ridx),w);
      recHitEnergyHE3s->Fill((*readRecHitsHE3_En).at(ridx),w);
      recHitEnergyHE->Fill((*readRecHitsHE3_En).at(ridx),w);
      recHitEnergyHEs->Fill((*readRecHitsHE3_En).at(ridx),w);

      recHitEnergyRAWHE3->Fill((*readRecHitsHE3_EnRAW).at(ridx),w);
      recHitEnergyRAWHE3s->Fill((*readRecHitsHE3_EnRAW).at(ridx),w);
      recHitEnergyRAWHE->Fill((*readRecHitsHE3_EnRAW).at(ridx),w);
      recHitEnergyRAWHEs->Fill((*readRecHitsHE3_EnRAW).at(ridx),w);

      if((*readRecHitsHE3_EnRAW).at(ridx) > 0.1) recHitHEM0M2->Fill((*readRecHitsHE3_EnRAW).at(ridx),(*readRecHitsHE3_En).at(ridx),w);
      if((*readRecHitsHE3_EnRAW).at(ridx) > 0.1) recHitHEM0M2Ratio->Fill((*readRecHitsHE3_EnRAW).at(ridx),(*readRecHitsHE3_En).at(ridx)/(*readRecHitsHE3_EnRAW).at(ridx),w);
      if((*readRecHitsHE3_EnRAW).at(ridx) > 0.1) recHitHEM2M0Dist->Fill((*readRecHitsHE3_EnRAW).at(ridx),(*readRecHitsHE3_En).at(ridx)/(*readRecHitsHE3_EnRAW).at(ridx),w);

      recHitTimingHE3->Fill((*readRecHitsHE3_time).at(ridx),w);
      recHitTimingHE3s->Fill((*readRecHitsHE3_time).at(ridx),w);

      recHitOccupancyHE3->Fill((*readRecHitsHE3_ieta).at(ridx),(*readRecHitsHE3_iphi).at(ridx),w);
      recHitTimingMapHE3->Fill((*readRecHitsHE3_ieta).at(ridx),(*readRecHitsHE3_iphi).at(ridx),(*readRecHitsHE3_time).at(ridx),w);
      recHitEnergyMapHE3->Fill((*readRecHitsHE3_ieta).at(ridx),(*readRecHitsHE3_iphi).at(ridx),(*readRecHitsHE3_En).at(ridx),w);
      if((*readRecHitsHE3_En).at(ridx) > 0.8){
	recHitEnergyCutHE->Fill((*readRecHitsHE3_En).at(ridx),w);
	recHitEnergyCutHEs->Fill((*readRecHitsHE3_En).at(ridx),w);
      }
    ...
    h_nRecHitsHE3->Fill(nRecHitsHE3,w);
    h_nRecHitsHE3s->Fill(nRecHitsHE3,w);

    h_nRecHitsHE->Fill(nRecHitsHE,w);
    h_nRecHitsHEs->Fill(nRecHitsHE,w);

    for(int i = 0; i < 13; i++) HE_RecHitNum_ieta_plus->Fill(i+16,nRecHitsHE_ieta[i],w);
    for(int i = 0; i < 13; i++) HE_RecHitCutNum_ieta_plus->Fill(i+16,nRecHitsCutHE_ieta[i],w);
    ...
    */
    ...
    nRecHitsHF1 = 0;
     for(int ridx = 0; ridx < (*readRecHitsHF1_En).size(); ridx++){
      recHitEnergyHF1->Fill((*readRecHitsHF1_En).at(ridx),w);
      recHitEnergyHF1s->Fill((*readRecHitsHF1_En).at(ridx),w);
      recHitEnergyHF->Fill((*readRecHitsHF1_En).at(ridx),w);
      recHitEnergyHFs->Fill((*readRecHitsHF1_En).at(ridx),w);

      recHitTimingHF1->Fill((*readRecHitsHF1_time).at(ridx),w);
      recHitTimingHF1s->Fill((*readRecHitsHF1_time).at(ridx),w);

      recHitOccupancyHF1->Fill((*readRecHitsHF1_ieta).at(ridx),(*readRecHitsHF1_iphi).at(ridx),w);
      recHitTimingMapHF1->Fill((*readRecHitsHF1_ieta).at(ridx),(*readRecHitsHF1_iphi).at(ridx),(*readRecHitsHF1_time).at(ridx),w);
      recHitEnergyMapHF1->Fill((*readRecHitsHF1_ieta).at(ridx),(*readRecHitsHF1_iphi).at(ridx),(*readRecHitsHF1_En).at(ridx),w);
      if((*readRecHitsHF1_En).at(ridx) > 0.5){
	recHitEnergyCutHF->Fill((*readRecHitsHF1_En).at(ridx),w);
	recHitEnergyCutHFs->Fill((*readRecHitsHF1_En).at(ridx),w);
      }
	...
	recHitSumAllHF1_ieta_plus[recHitIeta] += recHitEnergy*w;
      }
      if(recHitEnergy > 4 && recHitIeta < 0){
	recHitSumAllHF1_ieta_minus[-recHitIeta] += recHitEnergy*w; 
      }
	...
	if((longFiberIeta == shortFiberIeta) && (longFiberIphi == shortFiberIphi)){
      ...
      if(longFiberIeta > 0){
	if(long5)  fiberRatio_ieta_long5_plus->Fill(idx, ratio,w);
	if(long10) fiberRatio_ieta_long10_plus->Fill(idx,ratio,w);

	if(sum5)  fiberRatio_ieta_sum5_plus->Fill(idx,ratio,w);
	if(sum10) fiberRatio_ieta_sum10_plus->Fill(idx,ratio,w);

	if(long5&&sum10) fiberRatio_ieta_long5sum10_plus->Fill(idx,ratio,w);

	if(longFiberEnergy > .5){
	  if(sum5)  fiberRatio_ieta_sum5Threshold_plus->Fill(idx,ratioThreshold,w);
	  if(sum10) fiberRatio_ieta_sum10Threshold_plus->Fill(idx,ratioThreshold,w);
	}
      }
      if(longFiberIeta < 0){
	if(long5)  fiberRatio_ieta_long5_minus->Fill(idx, ratio,w);
	if(long10) fiberRatio_ieta_long10_minus->Fill(idx,ratio,w);

	if(sum5)  fiberRatio_ieta_sum5_minus->Fill(idx,ratio,w);
	if(sum10) fiberRatio_ieta_sum10_minus->Fill(idx,ratio,w);

	if(long5&&sum10) fiberRatio_ieta_long5sum10_minus->Fill(idx,ratio,w);

	if(longFiberEnergy > .5){
	  if(sum5)  fiberRatio_ieta_sum5Threshold_minus->Fill(idx,ratioThreshold,w);
	  if(sum10) fiberRatio_ieta_sum10Threshold_minus->Fill(idx,ratioThreshold,w);
	}
      }
    ...
    h_nRecHitsHF1->Fill(nRecHitsHF1,w);
    h_nRecHitsHF1s->Fill(nRecHitsHF1,w);

    //RecHit Energy HF - Depth 2
    nRecHitsHF2 = 0;
     for(int ridx = 0; ridx < (*readRecHitsHF2_En).size(); ridx++){
      recHitEnergyHF2->Fill((*readRecHitsHF2_En).at(ridx),w);
      recHitEnergyHF2s->Fill((*readRecHitsHF2_En).at(ridx),w);
      recHitEnergyHF->Fill((*readRecHitsHF2_En).at(ridx),w);
      recHitEnergyHFs->Fill((*readRecHitsHF2_En).at(ridx),w);

      recHitTimingHF2->Fill((*readRecHitsHF2_time).at(ridx),w);
      recHitTimingHF2->Fill((*readRecHitsHF2_time).at(ridx),w);

      recHitOccupancyHF2->Fill((*readRecHitsHF2_ieta).at(ridx),(*readRecHitsHF2_iphi).at(ridx),w);
      recHitTimingMapHF2->Fill((*readRecHitsHF2_ieta).at(ridx),(*readRecHitsHF2_iphi).at(ridx),(*readRecHitsHF2_time).at(ridx),w);
      recHitEnergyMapHF2->Fill((*readRecHitsHF2_ieta).at(ridx),(*readRecHitsHF2_iphi).at(ridx),(*readRecHitsHF2_En).at(ridx),w);
      if((*readRecHitsHF2_En).at(ridx) > 0.85){
	recHitEnergyCutHF->Fill((*readRecHitsHF2_En).at(ridx),w);
	recHitEnergyCutHFs->Fill((*readRecHitsHF2_En).at(ridx),w);
      }
      ...
      //Lev's Method
      if(recHitEnergy > 4 && recHitIeta > 0){
	recHitSumAllHF2_ieta_plus[recHitIeta] += recHitEnergy*w;
      }

      if(recHitEnergy > 4 && recHitIeta < 0){
	recHitSumAllHF2_ieta_minus[-recHitIeta] += recHitEnergy*w;
      }
    ...
    h_nRecHitsHF2->Fill(nRecHitsHF2,w);
    h_nRecHitsHF2s->Fill(nRecHitsHF2,w);

    h_nRecHitsHF->Fill(nRecHitsHF,w);
    h_nRecHitsHFs->Fill(nRecHitsHF,w);

    //RecHit Energy HO
    nRecHitsHO = 0;
     for(int ridx = 0; ridx < (*readRecHitsHO_En).size(); ridx++){
      recHitEnergyHO->Fill((*readRecHitsHO_En).at(ridx),w);
      recHitEnergyHOs->Fill((*readRecHitsHO_En).at(ridx),w);

      recHitTimingHO->Fill((*readRecHitsHO_time).at(ridx),w);
      recHitTimingHOs->Fill((*readRecHitsHO_time).at(ridx),w);

      recHitOccupancyHO->Fill((*readRecHitsHO_ieta).at(ridx),(*readRecHitsHO_iphi).at(ridx),w);
      recHitTimingMapHO->Fill((*readRecHitsHO_ieta).at(ridx),(*readRecHitsHO_iphi).at(ridx),(*readRecHitsHO_time).at(ridx),w);
      recHitEnergyMapHO->Fill((*readRecHitsHO_ieta).at(ridx),(*readRecHitsHO_iphi).at(ridx),(*readRecHitsHO_En).at(ridx),w);
      nRecHitsHO = nRecHitsHO + 1;
      numRecHits++;
    }
    h_nRecHitsHO->Fill(nRecHitsHO,w);
    h_nRecHitsHOs->Fill(nRecHitsHO,w);

    h_nRecHits->Fill(numRecHits,w);
    h_nRecHitss->Fill(numRecHits,w);
    ...
    for(int i = 1; i < 86; i++){
      if(true){
	energyFlow_plus->Fill(i,recHitSum_ieta_plus[i],w);
	if((i >= 16)&&(i <= 28)) HE_RecHitMeanEnergy_ieta_plus->Fill(i,(nRecHitsHE_ieta[i-16] > 0 ? recHitSum_ieta_plus[i]/nRecHitsHE_ieta[i-16] : 0),w);
	if((i >= 16)&&(i <= 28)) HE_RecHitCutMeanEnergy_ieta_plus->Fill(i,(nRecHitsCutHE_ieta[i-16] > 0 ? recHitSumCut_ieta_plus[i]/nRecHitsCutHE_ieta[i-16] : 0),w);
	
	if(i < 42){
	  theta = 2*atan(exp(-towerCenter[i]));
	  energyFlow_eta_plus->Fill(i,recHitSum_ieta_plus[i]/towerWidth[i],w);
	  if(debug && (bin == i)) cout << "RecHit Sum (All): " << recHitSum_ieta_plus[i] << endl;
	  energyFlowEt_eta_plus->Fill(i,recHitSum_ieta_plus[i]*sin(theta)/towerWidth[i],w);

	  energyFlowCut_eta_plus->Fill(i,recHitSumCut_ieta_plus[i]/towerWidth[i],w);
	  energyFlowCutEt_eta_plus->Fill(i,recHitSumCut_ieta_plus[i]*sin(theta)/towerWidth[i],w);
	  if (debug && (bin == i)) cout << "RecHit Fill: " << recHitSumCut_ieta_plus[i]*sin(theta)/towerWidth[i] << endl;
	}
      }
      if(true){
	energyFlow_minus->Fill(i,recHitSum_ieta_minus[i],w);
	if(i < 42){
	  theta = 2*atan(exp(-towerCenter[i]));
	  energyFlow_eta_minus->Fill(i,recHitSum_ieta_minus[i]/towerWidth[i],w);

	  energyFlowEt_eta_minus->Fill(i,recHitSum_ieta_minus[i]*sin(theta)/towerWidth[i],w);

	  energyFlowCut_eta_minus->Fill(i,recHitSumCut_ieta_minus[i]/towerWidth[i],w);
	  energyFlowCutEt_eta_minus->Fill(i,recHitSumCut_ieta_minus[i]*sin(theta)/towerWidth[i],w);
	}
      }

      if(true){
	caloFlowEm_plus->Fill(i,caloEm_ieta_plus[i],w);
	if(i < 42) caloFlowEm_eta_plus->Fill(i,caloEm_ieta_plus[i]/towerWidth[i],w);
      }
      if(true){
	caloFlowEm_minus->Fill(i,caloEm_ieta_minus[i],w);
	if(i < 42) caloFlowEm_eta_minus->Fill(i,caloEm_ieta_minus[i]/towerWidth[i],w);
      }

      if(true){
	caloFlowHad_plus->Fill(i,caloHad_ieta_plus[i],w);
	if(i < 42){
	  caloFlowHad_eta_plus->Fill(i,caloHad_ieta_plus[i]/towerWidth[i],w);
	  if(debug && (i == bin)) cout << "Calo Fill: " << caloHad_ieta_plus[i]/towerWidth[i] << endl;
	}
      }
      if(true){
	caloFlowHad_minus->Fill(i,caloHad_ieta_minus[i],w);
	if(i < 42) caloFlowHad_eta_minus->Fill(i,caloHad_ieta_minus[i]/towerWidth[i],w);
      }
