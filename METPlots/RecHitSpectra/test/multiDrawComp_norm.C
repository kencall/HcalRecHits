...
// KH: Taken form Ken Call's root script, but
//     normalize distributions by GetSumOfWeights of nVertx

void multiDrawComp_norm(const char *file1, const char *dataset1, const char *file2, const char* dataset2,
  ...
  TH1F *h1_norm = (TH1F*)f1->Get("nVertx");
  h1->Scale(1./h1_norm->GetSumOfWeights());
  
  f2->cd("");

  TH1F *h2 = (TH1F*)f2->Get(histname);
  TH1F *h2_norm = (TH1F*)f2->Get("nVertx");
  h2->Scale(1./h2_norm->GetSumOfWeights());
