...
//This function converts a 2017 depth to legacy
int convertDepth(int depth, int ieta){
	ieta = (ieta < 0 ? -ieta : ieta);

        //ieta less that 16 doesn't have any changes to the depth segmentation
	if(ieta < 16)  return depth;
	...
	if(ieta >= 18 && ieta <= 25){
		...
		if(depth >=3 && depth <=6) return 2;

		return -1;
	}
	if(ieta == 26){
		if(depth >=1 && depth <=3) return 1;
		if(depth >=4 && depth <=7) return 2;
...
//from the 2017 depth segmentation to the legacy depth segmentation
//The rechit energies of combined depths are simply added together
void convert2017toLegacy(const char *infile, const char *outfile){
...
//			float enRaw = (*readRecHitsHE_enRaw).at(ridx);
			...
			int legacyDepth = convertDepth(depth, ieta);

			if(legacyDepth < 1){
				cout << "Unable to convert ieta: " << ieta << " depth: " << depth << endl;
				continue;
			}
			if(legacyDepth > 3) cout << "ieta: " << ieta << " iphi: " << " depth: " << legacyDepth << endl;	
